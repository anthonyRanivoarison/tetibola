...
import {useFetch} from "../hooks/api";  // comme tu fais pour Expense
...
const NewIncome: React.FC = () => {
  ...
  const {data: incomeData, error, loading} = useFetch({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setStatus("error");
      setMessage(err?.response?.data?.message || "Failed to save income");
    }
  };
